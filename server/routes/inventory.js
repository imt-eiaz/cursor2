  ...
  try {
    const result = await db.query(`
      SELECT i.*, inv.quantity, inv.min_stock_level, inv.last_updated,
             CASE 
               WHEN inv.quantity <= inv.min_stock_level THEN 'Low Stock'
...
router.put('/:id', [
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE inventory SET ${updateFields.join(', ')} WHERE item_id = $${paramCount} RETURNING *`,
      updateValues
    );
...
router.post('/:id/add-stock', [
  ...
  try {
    ...
    console.log(`Stock added: Item ID ${id}, Quantity: ${quantity}, Notes: ${notes || 'No notes'}`);
...
router.post('/:id/remove-stock', [
  ...
  try {
    ...
    // Log stock removal
    console.log(`Stock removed: Item ID ${id}, Quantity: ${quantity}, Reason: ${reason}, Notes: ${notes || 'No notes'}`);
